...
$ModelOrderKitchen = 'MaxComanda/model/order-kitchen/order-kitchen-model.php';

$parametro = 's';
$tag = '';
while ($parametro != 'n') {
    if (file_exists($tag . $ModelOrderKitchen)) {
        $parametro = 'n';
    } else {
        $tag = '../' . $tag;
    }
}
$ModelOrderKitchen = $tag . $ModelOrderKitchen;
include_once($ModelOrderKitchen);
...
if ($ROW_Perm_Monitor_Kitchen->search == 'N' && $ROW_Perm_Monitor_Kitchen->include == 'N' && $ROW_Perm_Monitor_Kitchen->edit == 'N') {
                    ...
                    <div class="card-content">
                        ...
                        <table class="highlight centered responsive-table">
                            ...
                            <tbody>
                                ...
                                $listItems = "SELECT a.id, a.quantity, a.table_delivery, a.observation, a.table_demand, b.name, a.product_id 
                                FROM order_items a 
                                LEFT JOIN product b ON a.product_id = b.id 
                                WHERE a.uniqID = '$orderUniqID' $AND_table_delivery AND a.kitchen_status = 'Aguardando' AND a.temp = 2 ";
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <?php if ($ROW_Perm_Monitor_Kitchen->edit == 'S') { ?>

                                                <a class="btn-floating btn waves-effect waves-light blue tooltipped" data-tooltip="Pedido Pronto" onclick="finishOrder('<?php echo $rowItems->id; ?>')"><i class="material-icons">done</i></a>
