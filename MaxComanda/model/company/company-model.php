			...
			if (in_array($file_extension, $image_ext)) {
				...
				if (move_uploaded_file($_FILES['logo']['tmp_name'], $location)) {
					...
					if (!empty($id)) {
						...
						if ($rowOldImageCompany = $sqlOldImageCompany->fetch()) {
		...
		:origin)";
			...
			if (!empty($bloq)) {
				...
				if ($SQL_id_empresa->rowCount() > 0) {
					...
					$retorno = array('codigo' => 2, 'mensagem' => 'Empresa Cadastrada com Sucesso! Agora faça o login novamente! ' . $_SESSION['id_user']);
					echo json_encode($retorno);
					exit();
				}
			} else {
