...
if ( !empty( $listTables ) ) {
  $SQL_list_tables = "SELECT  a.uniqid, a.id AS id_mesa,SEC_TO_TIME(TIME_TO_SEC(a.last_update) - TIME_TO_SEC(NOW())) AS horas,(SELECT COUNT(*) FROM order_items y WHERE y.uniqID = a.uniqID) AS comandas,
a.status_table,
...
// DELETAR O PRODUTO TEMPORARIAMENTE - LEÔNIDAS MONTEIRO - 01/02/2022
	if(!empty($_GET['delProductTem'])){
		$temp = anti_injection($_POST['temp']);
