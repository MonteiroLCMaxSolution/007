...
// ******* LISTAR OS PERFIS CADASTRADO PARA A EMPRESA - MONTEIRO - 27/04/2022 *******
$SQL_list_profile = "SELECT a.id, a.name FROM profile a WHERE a.id_company = '1' AND a.`status` = 'ATIVO' AND a.id_contract = '37'";
$SQL_list_profile = $pdo->prepare($SQL_list_profile);
$SQL_list_profile->execute();

// ******* FIM LISTAR OS PERFIS CADASTRADO PARA A EMPRESA *******
...
if (isset($_GET['searchProfile'])) {
	...
	if (!empty($profileName)) {
		$WHERE_profileName = "WHERE a.id_company = ".$id_company." AND a.id_contract = ".$id_contract." AND name like '%$profileName%'";
	} else {
		$WHERE_profileName = "WHERE a.id_company = ".$id_company." AND a.id_contract = ".$id_contract;
	}

	$sqlSearchProfile = "SELECT * FROM profile a WHERE a.id_company = :id_company AND a.id_contract = :id_contract AND a.name LIKE :name";
	$sqlSearchProfile = $pdo->prepare($sqlSearchProfile);
	$sqlSearchProfile->bindValue('id_company',$id_company);
	$sqlSearchProfile->bindValue('id_contract',$id_contract);
	$sqlSearchProfile->bindValue('name','%'.$profileName.'%');
