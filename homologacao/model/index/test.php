...
if (isset($_GET['registerCompany'])) {
	...
	:origin)";
			...
			$addUserCompany = "INSERT INTO user_company (status, id_user, id_company) VALUES (:status, :id_user, :id_company)";
		...
		:origin)";
			...
			if (!empty($company)) {
				...
				while ($count != $countStart) {
					...
					$sqlAddUserCompany = "INSERT INTO user_company(id_user,id_company,status)VALUES(:id_user,:id_company,:status);";
...
// *********************** EXIBIR DADOS NO FORMULÁRIO - BRUNO R. BERNAL - 20/05/2022 ************************
...
if (isset($_GET['idUser'])) {
  ...
  LEFT JOIN profile d ON a.profile = d.id
	WHERE a.id = :id AND a.id_company = :id_company AND a.id_contract = :id_contract";
	$sqlListData = $pdo->prepare($sqlListData);
	$sqlListData->bindValue('id', $_GET['idUser']);
	$sqlListData->bindValue('id_company', $_COOKIE['id_company']);
...
if (!empty($_GET['access'])) {
	...
	$SQL_login = "SELECT  a.folder, a.id_company, a.id AS user_id,a.profile,a.img, a.`status`, a.password,  b.id_contract,a.name AS user_name, a.email AS user_mail, a.number_access AS user_access, a.last_access AS user_last, a.id_contract AS id_contract, b.name_razao_social AS company_name, b.color_header, b.color_text, b.logo  FROM user a 
LEFT JOIN company b ON a.id_contract = b.id_contract

WHERE a.email = :email AND a.folder = :folder;";
	$SQL_login = $pdo->prepare($SQL_login);
	$SQL_login->bindValue('email', $login);
	$SQL_login->bindValue('folder', $folder);
	$SQL_login->execute();
	if ($SQL_login->rowCount() >= 0) {
		...
		if ($status == "Ativo") {
        	...
        	$return = array('code' => 2, 'message' => "Acesso Permitido!","id_contract" => $id_contract, "user_id" => $user_id, "user_name" => $user_name, "user_mail" => $user_mail, "user_access" => $user_access, "user_last" => $user_last, "company_id" => $company_id, "company_name" => $company_id, 'user_id' => $user_id);
        	echo json_encode($return);
        	exit();
        } else {
        	$return = array('code' => 1, 'message' => "É necessário informar os dados de sua empresa!","id_contract" => $id_contract, "user_id" => $user_id, "user_name" => $user_name, "user_mail" => $user_mail, "user_access" => $user_access, "user_last" => $user_last, "company_id" => $company_id, "company_name" => $company_id, 'user_id' => $user_id);
        	echo json_encode($return);
        	exit();
        }*/
				if (!empty($row->company_name)) {
					...
					$return = array('code' => 2, 'message' => "Acesso Permitido! " . $_SESSION['id_contract'] . " Acesso: " . $user_id, "id_contract" => $id_contract, "user_id" => $user_id, "user_name" => $user_name, "user_mail" => $user_mail, "user_access" => $user_access, "user_last" => $user_last, "company_id" => $company_id, "company_name" => $company_id, 'user_id' => $user_id);
					echo json_encode($return);
					//exit();
				} else {
				...
				$description = $login . ", ACESSOU O SISTEMA COM A SENHA INVÁLIDA:" . $password;
				$mail = $login;
				$return = array('code' => CURL_SSLVERSION_TLSv1, 'message' => "Senha é inválida!");
				echo json_encode($return);
				//exit();
			}
		} else {
			$title = "ACESSO NEGADO!";
			$description = $login . ", STATUS INATIVO!";
			$mail = $login;
			$return = array('code' => 1, 'message' => "Verifique sua permissão junto a sua Gerencia!");
			echo json_encode($return);
			exit();
		}
	} else {
		$title = "LOGIN OU E-MAIL NÃO ENCONTRADO!";
		$description = $login . ", NÃO ENCONTRADO!";
		$mail = $login;
		$return = array('code' => 0, 'message' => "E-mail não encontrado!");
		echo json_encode($return);
		//exit();
	}
