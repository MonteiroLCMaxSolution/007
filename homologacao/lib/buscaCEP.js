function buscaCEP(func = null, origin = null) {
    var cep = $("#CEP").val();

    // SE O PARAMETRO "ORIGIN" ESTIVER PREENCHIDO, SIGNIFICA QUE A FUNÇÃO FOI CHAMADA APÓS O BUSCACNPJ, PORTANTO, NÃO DEVE LIMPAR OS CAMPOS DE ENDEREÇO, CASO O RETORNO SEJA VAZIO
    ...
    if (cep.length == 9) {
        ...
        $.ajax({
            ...
            success: function (resposta) {
                ...
                if (origin == null || $("#address").val() == "") {
                    $("#address").val(resposta.logradouro);
                }

                if (origin == null || $("#complement").val() == "") {
                    $("#complement").val(resposta.complemento);
                }

                if (origin == null || $("#district").val() == "") {
                    $("#district").val(resposta.bairro);
                }

                if (origin == null || $("#number").val() == "") {
                    $("#number").val("");
                }
